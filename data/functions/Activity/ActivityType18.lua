module("ActivityType18", package.seeall)
...
ActivityType = 18
--对应的活动配置
ActivityConfig = Activity18Config
if ActivityConfig == nil then 
    print("Activity18Config errer!") 
    --assert(false)
end

--排行榜id
RANKING_ID = RANK_DEFINE_ACTIVITY23
...
function reqEnterFuben(pActor, atvId)
   ...
   if fbHandle then
        ...
        --print("[ActivityType18:reqEnterFuben-------------------------------------------------------------reqEnterFuben".. tostring(atvId))
        -- 记录日志
...
function reqGetPhaseAward(pActor, atvId)
    ...
    --print("[ActivityType18:reqGetPhaseAward-------------------------------------------------------------reqGetPhaseAward")
    if actorAwardIdx[atvId] == nil then
        actorAwardIdx[atvId] = {}
    end

    if actorAwardIdx[atvId][aid] == nil then
        actorAwardIdx[atvId][aid] = 1                                               --如果没有 ，找第一阶段的奖励
    end
    ...
    if awardConf then
        ...
        local curPhaseVal = RankMgr.GetValue(actorid, RANKING_ID)
        if curPhaseVal >= awardConf.value then
            ...
            CommonFunc.Awards.Give(pActor, awards, GameLog.Log_ActivityWMSAcross, "Activity18LevelAward|"..atvId)
            actorAwardIdx[atvId][aid] = actorAwardIdx[atvId][aid] + 1               --加1 找下一阶段的奖励
...
function BroadRankData(atvId,fbHandle,sceneId)
    --print("[ActivityType18:BroadRankData1-------------------------------------------------------------BroadRankData1")
    -- 广播所有玩家排行榜数据
...
function SendRankData(atvId,pActor)
    --print("[ActivityType18:SendRankData-------------------------------------------------------------SendRankData")
...
function BroadCurrentTimeInfo(atvId,fbHandle,sceneId)
    --print("[ActivityType18:BroadCurrentTimeInfo-------------------------------------------------------------BroadCurrentTimeInfo")
...
function BroadAssignCampTimeInfo(atvId,fbHandle,sceneId)
    --print("[ActivityType18:BroadAssignCampTimeInfo---------------------------------------------------------BroadAssignCampTimeInfo")
        ...
        if ranking then
            ...
            for _,awardInfo in ipairs(rankAward) do
                ...
                for i=idx,rankNum do
                    ...
                    --local title = "跨服夜战沃玛三"
                    local content = string.format(ActivityConfig[atvId].mailContent1, i)
                    --SendMail(actorId, title, content, awardInfo.awards)
                    SendCrossServerMail(actorId, ActivityConfig[atvId].mailTitle1, content, awardInfo.awards)
                    local pActor = Actor.getActorById(actorId)
                    local name = RankMgr.GetValueById(actorId, RANKING_ID, 0)
                    --print("[GActivity 11] 跨服夜战沃玛三(id:"..atvId..") "..(name or actorId).." 获得第"..i.."名！")
                end
                idx = rankNum + 1
                if idx > itemNum then
                    --print("[GActivity 11] 跨服夜战沃玛三(id:"..atvId..") 奖励发完! 一共"..rankNum.."人获得上榜奖励!")
                    return
                end
            end
        end
...
function SendScoreAward(atvId)
    ...
    --print("[ActivityType18:SendScoreAward-------------------------------------------------------------reqGetPhaseAward"..tostring(atvId)..":"..tostring(MaxIdx))
    -- local title = "夜战沃玛三未领奖励"
    if actorAwardIdx[atvId] == nil then
        actorAwardIdx[atvId] = {}
    end 
    for aid, awdIdx in pairs(actorAwardIdx[atvId]) do
        --print("[ActivityType18:----------------------------------------"..tostring(aid))
            
        if actorAwardIdx[atvId][aid] ~= nil then 
            local rankValue = RankMgr.GetValue(aid, RANKING_ID)
           -- print("[ActivityType18:rankValue----------------------------------------"..tostring(rankValue))
            for i = MaxIdx, 1 , -1 do  
                local awardConf = ActivityConfig[atvId].phaseAward[i]
                --print("[ActivityType18:ActivityConfig[atvId].phaseAward[i]----------------------------------------"..tostring(rankValue)..":"..tostring(awardConf.value)..":"..tostring(i))
                    
                if awardConf then
                    
                    if rankValue >= awardConf.value then
                        local content = string.format("你在跨服夜战沃玛三中还有积分%d档的奖励未领取！", awardConf.value)
                        --SendMail(aid, ActivityConfig[atvId].title, content, awardConf.awards)
                        SendCrossServerMail(aid, ActivityConfig[atvId].mailTitle2, ActivityConfig[atvId].mailContent2, awardConf.awards)
                        break
...
function OnLoad(atvId)
    --print("[ActivityType18:OnLoad----------------------------------------------------------------------- OnLoad:".. tostring(atvId))
...
function OnStart(atvId)
    ...
    --print("[ActivityType18:OnStart----------------------------------------------------------------------- OnStart:".. tostring(atvId))
    ...
    -- 广播第一名
     ...
     if ranking then
         ...
         local name = Ranking.getSub(rankItem, 0)
            if rankItem then
                System.broadcastTipmsgLimitLev("跨服夜战沃玛三活动结束了，恭喜玩家 "..name.." 获得第一名！", tstRevolving)
                System.broadcastTipmsgLimitLev("跨服夜战沃玛三活动结束了，恭喜玩家 "..name.." 获得第一名！", tstChatSystem)
            end 
     end
...
function OnOperator(atvId, pActor, inPack)      
    ...
    --print("[ActivityType18:OnOperator----------------------------------------------------------------------- OnOperator:".. tostring(operaCode))
    ...
    elseif operaCode == ActivityOperate.cGetPhaseAward then --请求获取阶段奖励
        return
        --跨服不需要实时领取，最后统一发送 reqGetPhaseAward(pActor, atvId)
    end
end
        ...
        if killerType == enActor then
            if entityType == enActor then
                ...
                elseif actorDataKiller.MaxMultiHit ==8 then 
                    System.broadcastTipmsgLimitLev(Actor.getName(pKiller).."无人可挡，在跨服夜战沃玛三取得了八连胜！", tstRevolving)
                elseif actorDataKiller.MaxMultiHit ==5 then
                    System.broadcastTipmsgLimitLev(Actor.getName(pKiller).."主宰比赛，在跨服夜战沃玛三取得了五连胜！", tstRevolving)
                elseif actorDataKiller.MaxMultiHit ==3 then 
                    System.broadcastTipmsgLimitLev(Actor.getName(pKiller).."在跨服夜战沃玛三取得了三连胜！", tstRevolving)
                end
...
function OnLoginGame(atvId,pActor)
    ...
    --print("[ActivityType18:OnLoginGame----------------------------------------------------------------------- OnLoginGame:"..  tostring(atvId)..":"..tostring( actorData.EndTime))
    --System.CheckGPActivityStartEnd(pActor, atvId, actorData.EndTime)

end 
ActivityDispatcher.Reg(ActivityEvent.OnGetRedPoint, ActivityType, OnGetRedPoint, "ActivityType18.lua")

ActivityDispatcher.Reg(ActivityEvent.OnLoginGame, ActivityType, OnLoginGame, "ActivityType18.lua")

ActivityDispatcher.Reg(ActivityEvent.OnLoad, ActivityType, OnLoad, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnInit, ActivityType, OnInit, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnStart, ActivityType, OnStart, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnEnd, ActivityType, OnEnd, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnUpdate, ActivityType, OnUpdate, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnReqData, ActivityType, OnReqData, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnOperator, ActivityType, OnOperator, "ActivityType18.lua")  
ActivityDispatcher.Reg(ActivityEvent.OnExitFuben, ActivityType, OnExitFuben, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnEntityDeath, ActivityType, OnEntityDeath, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnEntityAttacked, ActivityType, OnEntityAttacked, "ActivityType18.lua")
ActivityDispatcher.Reg(ActivityEvent.OnFubenFinish, ActivityType, OnFubenFinish, "ActivityType18.lua")
